...
var _ = Describe("update command", func() {
	When("--self-update=yes", func() {
		...
		It("should be self-updated", func() {
			...
			if runtime.GOOS == "windows" {
				multiwerfExeOldFilePath := filepath.Join(multiwerfTmpDir, fmt.Sprintf(".%s.old", filepath.Base(multiwerfForSelfUpdateBinPath)))
				Ω(multiwerfExeOldFilePath).Should(BeARegularFile(), fmt.Sprintf(".%s.old file should remain for windows", filepath.Base(multiwerfForSelfUpdateBinPath)))
			} else {
	...
	When("--self-update=no", func() {
		...
		// * the release files for channel should be downloaded to v1.0.6-rc.6 folder
		...
		// * tmp dir should be empty
		baseEntry := ItEntry{
			multiwerfArgs: []string{"update", "1.0", "stable"},
			checksAfterFirstStep: func(output string) {
				for _, substr := range []string{
					"The version v1.0.6-rc.6 is the actual for channel 1.0/stable",
					"Downloading the version v1.0.6-rc.6 ...",
				...
				releaseFilesShouldBeExist(actualAlphaVersion1)
				storageTmpDirShouldBeEmpty()
			},
			checksAfterSecondStep: func(output string) {
				for _, substr := range []string{
			...
			checksAfterThirdStep: func(output string) {
				for _, substr := range []string{
					"The version v1.0.6-rc.11 is the actual for channel 1.0/stable",
					"Downloading the version v1.0.6-rc.11 ...",
					"The actual version has been successfully downloaded",
				} {
					Ω(output).Should(ContainSubstring(substr))
				}
				...
				releaseFilesShouldBeExist(actualAlphaVersion2)
				storageTmpDirShouldBeEmpty()
			},
		}
